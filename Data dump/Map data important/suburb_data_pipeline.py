#!/usr/bin/env python3
"""
Suburb Data Pipeline (Queensland — Brisbane / Ipswich / Logan)
- Fetches locality & LGA polygons from QSpatial (fallback to ABS ASGS 2021).
- Derives centroids, label points, and distance to Brisbane CBD.
- Computes adjacency (shared-edge only) and nearest non-adjacent within same LGA.
- Writes base outputs: suburbs.geojson, suburbs.csv, adjacency.json, clusters.json, cluster_map.json, sources_index.json.
- Optionally enriches with ABS SA2/SA3/SA4/GCCSA + top-3 POA postcodes and writes suburbs_enriched.geojson/.csv.

Original content came from a notebook with repeated cells; this file removes duplicates and organizes functions.
"""
...
# Brisbane CBD reference (WGS84 lat, lon)
...
# Derived-neighbour backfill buffer (m)
...
# Build core dataset
# ------------------------------
def build_base_dataset() -> Tuple[gpd.GeoDataFrame, Dict[str, List[str]], Set[Tuple[str,str]], Dict[str, List[str]]]:
    """Returns loc (GeoDataFrame), adj_sorted, derived_edges, nearest_map."""
    ...
    # 9) Backfill “islands” to reach ≥2 neighbours (same LGA, within buffer)
    derived_edges = set()
    ...
    def sort_key(a_slug, b_slug):
        ...
        # recompute in meters CRS
        seg = loc_m.geometry[i].boundary.intersection(loc_m.geometry[j].boundary)
        length = float(getattr(seg, "length", 0.0))
        return (-length, b_slug)
    ...
    if few:
        print(f"⚠️ {len(few)} suburbs have <2 neighbours after backfill (edge cases):", few[:10])

    return loc, adj_sorted, derived_edges, nearest_map

# ------------------------------
# Writing outputs
# ------------------------------
def write_base_outputs(loc: gpd.GeoDataFrame,
                       adj_sorted: Dict[str, List[str]],
                       derived_edges: Set[Tuple[str,str]],
                       nearest_map: Dict[str, List[str]]) -> None:
    ...
    def to_props(r):
        return {
            ...
            "polygon_ref": ("ABS ASGS 2021 SAL (CC BY 4.0)" if r.get("source_used")=="ABS"
    ...
    (OUT/"suburbs.geojson").write_text(json.dumps({"type":"FeatureCollection","features":features}, ensure_ascii=False))
    ...
    } for p in props]).to_csv(OUT/"suburbs.csv", index=False)
    ...
    (OUT/"adjacency.json").write_text(json.dumps(adjacency_json, ensure_ascii=False, indent=2))
    ...
    (OUT/"clusters.json").write_text(json.dumps(clusters, ensure_ascii=False, indent=2))
    (OUT/"cluster_map.json").write_text(json.dumps(cluster_by, ensure_ascii=False, indent=2))
    ...
    (OUT/"sources_index.json").write_text(json.dumps({"generated_at": pd.Timestamp.utcnow().isoformat(), "sources": sources_idx}, ensure_ascii=False, indent=2))

    print("Wrote base outputs to", OUT.resolve())
...
# ------------------------------
# Optional: ABS enrichment (SA2/3/4, GCCSA, POA)
# ------------------------------
...
def enrich_with_abs(loc_df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
...
def write_enriched_outputs(enriched: gpd.GeoDataFrame) -> None:
    OUT = Path(OUTDIR); OUT.mkdir(exist_ok=True, parents=True)
    ...
    def props_enriched(r):
        return {
            ...
            "centroid": {"lat": round(r["centroid_lat"],6), "lon": round(r["centroid_lon"],6)},
            "label_point": {"lat": round(r["label_lat"],6), "lon": round(r["label_lon"],6)},
            "polygon_ref": "QSpatial or ABS (see sources_index.json)",
            "population_est": None,
            "population_year": 2021,
            "distance_to_bne_cbd_km": float(r["distance_to_bne_cbd_km"]),
            "transport_notes": None,
    ...
    for geom, p in zip(enriched.geometry, props2):
        gj = json.loads(gpd.GeoSeries([geom]).to_json())["features"][0]["geometry"]
        features.append({"type":"Feature","geometry":gj,"properties":p})
    (OUT/"suburbs_enriched.geojson").write_text(json.dumps({"type":"FeatureCollection","features":features}, ensure_ascii=False))
    ...
    pd.DataFrame([{
        ...
        "record_hash": p["record_hash"]
    } for p in props2]).to_csv(OUT/"suburbs_enriched.csv", index=False)

    print("Wrote enriched outputs to", OUT.resolve())

# ------------------------------
# Optional helpers: zip & download (for Colab)
# ------------------------------
def make_zip(dest_prefix: str = "suburb-data"):
    import datetime, shutil, subprocess
    folder = OUTDIR if os.path.isdir(OUTDIR) else None
    assert folder, "No output folder found — run the pipeline first."
    stamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
    zip_base = f"{dest_prefix}-{stamp}"
    zip_path = shutil.make_archive(zip_base, 'zip', root_dir=folder)
    print("Created zip:", zip_path)
    try:
        subprocess.run(["unzip", "-l", zip_path], check=False)
    except Exception:
        pass
    return zip_path
...
# Entry point
# ------------------------------
def main(do_enrich: bool = True):
    loc, adj_sorted, derived_edges, nearest_map = build_base_dataset()
    write_base_outputs(loc, adj_sorted, derived_edges, nearest_map)

    if do_enrich:
        enriched = enrich_with_abs(loc)
        write_enriched_outputs(enriched)
